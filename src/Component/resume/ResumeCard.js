...
const ResumeCard = ({ badge, title, subTitle, des }) => {
  return (
      <div className='w-full py-6 flex flex-col gap-2.5 border-b-[1px] border-zinc-800'>
        {badge === ""? (
        <h6 className='w-28 text-center text-sm py-[1px] text-designColor border-[1px] border-designColor rounded-sm'>
            2022-Present
        </h6>
        ):(
            <h6 className='w-24 text-center text-sm py-[1px] text-[#999]  border-zinc-600 rounded-sm'>
            {badge}
        </h6>
        )}
            ...
            <p className='text-base text-[#999] font-medium pr-10'>{des}
            </p>
        </div>
