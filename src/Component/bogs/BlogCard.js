...
const BlogCard = ({ image, title, subTitle, category}) => {
  return (
    ...
    <div className='w-full h-full mb-2 overflow-hidden relative cursor-pointer'>
      ...
      <div className='w-full h-full absolute top-0 left-0 hover:bg-gradient-to-t from-green-600 via-green-600 
            to-green-200 opacity-10'></div>
    </div>
    <div className='flex flex-col items-center gap-2'>
        <span className='text-sm text-designColor border-[1px] border-designColor px-2'>
            {title}
            </span>
            <h2 className='text-base text-center font-semibold font-titleFont text-[#ccc]
            group-hover:text-designColor duration-300 cursor-default'>
                {subTitle}</h2>
            <p>{category}</p>
    </div>
    </div>
